...
@Component({
  selector: 'app-store-t4',
  templateUrl: './store-t4.component.html',
  styleUrls: ['./store-t4.component.css']
})
export class StoreT4Component implements OnInit {
  count = 0;
  count2 = 0;
  count_weight = 0;
  count_weight_c = 0;
  ...
  ngOnInit() {
    ...
    this.detailFilter = [];
      ...
      for (let i = 0; i < a.length; i++) {
        ...
        && a[i].type !== 'หาง' && a[i].type !== 'ขา' && a[i].type !== 'ไขมัน' && a[i].type !== 'อองเร') && Number(a[i].hidden) === 0) {
          this.count_weight += Number(a[i].weight);
          this.count_weight_c += Number(a[i].weight_c);
          const toDay = new Date();
          const saveDay = new Date(a[i].date);
          const timeDiff = toDay.getTime() - saveDay.getTime();
        ...
        && a[i].type !== 'ซากซ้ายล่าง' && a[i].type !== 'ซากขวาบน' && a[i].type !== 'ซากขวาล่าง'
        && a[i].type !== 'เครื่องใน' && a[i].type !== 'หัว' && a[i].type !== 'หนัง'
        && a[i].type !== 'หาง' && a[i].type !== 'ขา' && a[i].type !== 'ไขมัน' && a[i].type !== 'อองเร') && Number(a[i].hidden) === 1) {
