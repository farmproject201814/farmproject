...
@Component({
  selector: 'app-report4-t1',
  templateUrl: './report4-t1.component.html',
  styleUrls: ['./report4-t1.component.css']
})
export class Report4T1Component implements OnInit {
  count = 0;
  count_weight = 0;
  count_weight_c = 0;
  datas: any = [];
  ...
  countReport = 0;
    ...
    start_at;
    end_at;
  constructor(private api: Menu4Service, private authAf: AngularFireAuth, private auth: AuthService,
  ...
  ngOnInit() {
    ...
    this.api.showStore().subscribe(data => {
      ...
      for (let i = 0; i < a.length; i++) {
        ...
        this.datass[i].day_store = Math.abs(diff);
      }
    });
     ...
     + this.t15_num + this.t16_num + this.t17_num + this.t18_num + this.t19_num + this.t20_num + this.t21_num + this.t22_num + this.t23_num;
      ...
      const docDefinition = {
        content: [
          ...
          {text: 'ประวัติคงคลัง', style: 'header2'},
  ...
  buildTableBody(data, columns) {
    const body = [];
    body.push(['ลำดับ', 'ประเภทซาก', 'รหัสบาร์โค้ด', 'น้ำหนักอุ่น(กก.)', 'น้ำหนักเย็น(กก.)',
     'เกรด', 'ห้อง', 'ชั้น', 'ตะกร้า', 'สถานะ', 'หมายเหตุ']);
...
searchDate() {
  ...
  console.log(this.endvalue);
  // tslint:disable-next-line:max-line-length
  firebase.database().ref().child('/store/menu4/store-t1').orderByChild('date').startAt(Number(this.startvalue)).endAt(Number(this.endvalue)).once('value', data => {
