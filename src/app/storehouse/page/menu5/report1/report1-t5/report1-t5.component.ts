...
@Component({
  selector: 'app-report1-t5',
  templateUrl: './report1-t5.component.html',
  styleUrls: ['./report1-t5.component.css']
})
export class Report1T5Component implements OnInit {
  name;
  count = 0;
  datas: any = [];
  datass = [];
  ...
  ngOnInit() {
    ...
    this.api.showHistory_Import().subscribe(datas => {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        if (value[i].type === 'เครื่องใน' || value[i].type === 'หัว' || value[i].type === 'หนัง'
