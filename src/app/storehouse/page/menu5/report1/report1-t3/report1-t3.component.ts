...
@Component({
  selector: 'app-report1-t3',
  templateUrl: './report1-t3.component.html',
  styleUrls: ['./report1-t3.component.css']
})
export class Report1T3Component implements OnInit {
  name;
  count = 0;
  datas: any = [];
  datass = [];
  ...
  ngOnInit() {
    ...
    this.api.showHistory_Import().subscribe(datas => {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        if (value[i].type === 'ซากซ้ายบน' || value[i].type === 'ซากซ้ายล่าง' ||
         value[i].type === 'ซากขวาบน' || value[i].type === 'ซากขวาล่าง')  {
          value[i].count = c + 1;
